...
def _derive_tile_coord(row: dict[str, str]) -> str:
    for key in ["tile_coord", "polygon_name", "county_name", "raster_path"]:
...
def _dominant_tillage(row: dict[str, str]) -> str:
    prop_0 = _to_float(_pick(row, "tillage_0_prop", "value_0_prop"))
    prop_1 = _to_float(_pick(row, "tillage_1_prop", "value_1_prop"))
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Normalize per-year tillage aggregate outputs into Lobell tillage field-year rows.")
    ...
    with (
        ...
        for row in reader:
            ...
            out_row = {
                ...
                "year": year,
                "dominant_tillage": _dominant_tillage(row),
                "tillage_0_count": _pick(row, "tillage_0_count", "value_0_count"),
                "tillage_1_count": _pick(row, "tillage_1_count", "value_1_count"),
                "tillage_na_count": _pick(row, "tillage_na_count", "value_na_count"),
                "tillage_0_prop": _pick(row, "tillage_0_prop", "value_0_prop"),
                "tillage_1_prop": _pick(row, "tillage_1_prop", "value_1_prop"),
                "tillage_na_prop": _pick(row, "tillage_na_prop", "value_na_prop"),
                "pixel_count": _pick(row, "pixel_count", "observed_count", "count"),
