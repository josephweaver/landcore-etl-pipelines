...
field-year-crop\docs\OS-003-local-synthetic-gorc-workflow.md
...
- If current GORC behavior blocks the OS, stop and append a concise blocker to field-year-crop/docs/issues.md. Do not patch GORC.

When complete:
1. Run the validation commands listed in the OS, or state exactly why they could not be run.
2. Update field-year-crop/docs/STATE.md.
3. Mark the OS as implemented in the OS file.
4. Add a short usage note to field-year-crop/docs/usage/<yyyyMMddHHmmss>-<os-id>.md using the local usage template if present.
```
