# OS-004: Local Real Inputs — CDL 2010 and Yan/Roy h18v07
...
```text
orchestration/workflows/local-real-input-metadata-2010.workflow.json
orchestration/submissions/local-real-input-metadata-2010.submission.json
orchestration/configs/local-worker.gdal.json
orchestration/scripts/python/discover_raster_asset.py
orchestration/scripts/python/run_raster_info.py
orchestration/scripts/smoke/local_real_input_metadata_2010.sh
...
If current GORC archive extraction requires explicit selected members and cannot extract the CDL directory generically, record a blocker in `orchestration/docs/issues.md` and implement a LandCore-side prefetch script as a temporary local pilot workaround.
...
```bash
bash orchestration/scripts/smoke/local_real_input_metadata_2010.sh
```
